...
def _func(k_neighbour_matrix, k_pred, em=None):
    ...
    for i in range(k_pred.shape[0]):
        ...
        for n_index in range(neighbour_index_len):
            ...
            res =  dtw(k_pred[i].astype(np.float32), emotion.astype(np.float32))
            dwt_list.append(res)
        min_dwt_sum += min(dwt_list)
    return min_dwt_sum


def compute_FRD_mp(args, pred, em, val_test='test', p=4):
    # pred: N 10 750 25
    # speaker: N 750 25
    ...
    return np.mean(FRD_list)*10



def compute_FRD(args, pred, listener_em, val_test='test'):
    if val_test == 'val':
    ...
    for i in range(pred.shape[1]):
        ...
        for k in range(pred.shape[0]):
            ...
            for n_index in range(speaker_neighbour_index_len):
                ...
                res =  dtw(pred[k, i].numpy().astype(np.float32), emotion.numpy().astype(np.float32))
                dwt_list.append(res)
            min_dwt = min(dwt_list)
            FRD_list.append(min_dwt)
        all_FRD_list.append(np.mean(FRD_list))
    return np.mean(all_FRD_list)*10