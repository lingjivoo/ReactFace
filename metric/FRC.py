...
def concordance_correlation_coefficient(y_true, y_pred,
    ...
    if len(y_true.shape) > 1:
        ...
        for i in range(y_true.shape[1]):
            ...
            ccc = numerator / (denominator + 1e-50)
...
def compute_FRC(args, pred, listener_em, val_test='test'):
    pred = pred
    ...
    return np.mean(all_FRC_list)*10
...
def compute_FRC_mp(args, pred, em, val_test='test', p=4):
    # pred: N 10 750 25
    # speaker: N 750 25
    if val_test == 'val':
        neighbour_matrix = np.load(os.path.join(args.dataset_path, 'neighbour_emotion_new_val.npy'))
    else:
        neighbour_matrix = np.load(os.path.join(args.dataset_path, 'neighbour_emotion_new_test.npy'))
    FRC_list = []
    with mp.Pool(processes=p) as pool:
        # use map
        _func_partial = partial(_func, em=em.numpy())
        FRC_list += pool.starmap(_func_partial, zip(neighbour_matrix, pred.numpy()))
    return np.mean(FRC_list)*10