...
class NeighbourLoss(nn.Module):
    ...
    def forward(self, gt_emotion_neighbours, gt_3dmm_neighbours, pred_emotion, pred_3dmm, distribution):
        rec_emotion = (torch.min(self.rec_loss(pred_emotion.unsqueeze(1).expand_as(gt_emotion_neighbours),
                                                  gt_emotion_neighbours).mean(dim=(2, 3)), dim=1)[0]).mean()
        ...
        rec_loss = rec_emotion + rec_3dmm_exp + 10 * rec_3dmm_pose

        mu_ref = torch.zeros_like(distribution[0].loc).to(gt_emotion_neighbours.get_device())
        scale_ref = torch.ones_like(distribution[0].scale).to(gt_emotion_neighbours.get_device())
...
class VAELoss(nn.Module):
    ...
    def forward(self, gt_emotion, gt_3dmm, pred_emotion, pred_3dmm, distribution):
        rec_loss = self.rec_loss(pred_emotion, gt_emotion) + self.rec_loss(pred_3dmm[:, :, :52], gt_3dmm[:, :,
                                                                                                 :52]) + 10 * self.rec_loss(
